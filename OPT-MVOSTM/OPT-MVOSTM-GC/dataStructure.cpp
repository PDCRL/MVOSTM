 ...
 */
void HashMap::list_Ins(int L_key, int *L_val, G_node** G_preds, 
 ...
 */
OPN_STATUS HashMap::list_LookUp(int L_bucket_id, int L_key, 
...
OPN_STATUS HashMap:: commonLuNDel(int L_tx_id, int L_bucket_id, int L_key, 
								  ...
								  G_node** G_currs)
{
    ...
    if(G_currs[1]->G_key == L_key)
    {
		...
		if(liveList->size() != 0 && liveList->at(0) == L_tx_id)
		{
			...
			G_node* m = G_preds[0]; 
			G_node* n = G_preds[0]->red_next;
			
			while(m->red_next != G_currs[1])
			{
				if((n->G_mark == true) and (n->G_vl->at(n->G_vl->size()-1)->max_rvl < L_tx_id)) {
					n->deletedMark = true;
					m->red_next = n->red_next;
				} else {
					m = n;
				}
				n = n->red_next;
...
void HashMap::insertVersion(int L_tx_id, int L_val, G_node* key_Node)
{
    ...
    if(liveList->size() != 0 && liveList->at(0) == L_tx_id)
    {
		memory_count.fetch_sub(key_Node->G_vl->size());
...
OPN_STATUS methodValidation(G_node** G_preds, G_node** G_currs)
{
    ...
    if((G_preds[0]->G_mark) || (G_currs[1]->G_mark) || 
		...
		(G_preds[1]->red_next->G_key != G_currs[0]->G_key) || G_preds[0]->deletedMark == true || G_currs[0]->deletedMark == true)
		return RETRY;
    else
        return OK;
}
