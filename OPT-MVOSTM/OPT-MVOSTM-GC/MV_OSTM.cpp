...
OPN_STATUS MV_OSTM::tx_lookup(L_txlog* txlog, int L_key, int* value)
{
    ...
    } else
    {
		...
		if(liveList->size() != 0 && liveList->at(0) == txlog->L_tx_id)
		{
			lockLiveList->unlock();
			return OK;
		}
...
OPN_STATUS MV_OSTM::tx_insert(L_txlog* txlog, int L_key, int L_val)
{
    ...
    if(record == NULL)
    {
		bool sanityCheck = hash_table->insert_sanity(txlog->L_tx_id,L_bucket_id,L_key);
...
OPN_STATUS MV_OSTM::tryCommit(L_txlog* txlog)
{
    ...
    {
		/* Sanity Check*/
		if((L_list->at(i)->getOpn() == LOOKUP) || (L_list->at(i)->getOpn() == DELETE && L_list->at(i)->getOpnStatus() == FAIL))
			continue;
